...
public class ComputerProjectsPR2Impl implements ComputerProjects, ComputerProjectsHelper {
    ...
    @Override
    public void addProject(String projectId, String name, String coordinatorId) {
    ...
    @Override
    public void registerTaskInProject(String taskId, String projectId, String description)
    ...
    // Dejamos los otros métodos helper para cuando implementemos las listas de
    // tareas
    @Override
    ...
    @Override
    public void assignTaskToOperatorTest(String operatorId, String taskId)
            throws OperatorNotFoundException, TaskNotFoundException, TaskAlreadyAssignedException {
        ...
        if (task.getStatus() != TaskStatus.REGISTERED && task.getStatus() != TaskStatus.REJECTED) {
    ...
    @Override
    public void acceptTaskByOperator(String operatorId, String taskId)
            throws OperatorNotFoundException, TaskNotAssignedToOperatorException {
        // 1. Buscamos al operador
        Operator operator = this.operators.get(operatorId);
        if (operator == null) {
            throw new OperatorNotFoundException("Operator " + operatorId + " not found.");
    ...
    public void doTaskByOperator(String operatorId) throws OperatorNotFoundException, NoAcceptedTasksException {
        ...
        // Este metodo saca (poll) de la cola 'accepted' y añade a 'completed'
    ...
    @Override
    public void rejectTaskByOperator(String operatorId, String taskId)
            throws OperatorNotFoundException, TaskNotAssignedToOperatorException, NoAssignedTasksException {
        // 1. Buscamos al operador
        Operator operator = this.operators.get(operatorId);
        if (operator == null) {
            throw new OperatorNotFoundException("Operator " + operatorId + " not found.");
    ...
    @Override
    public Iterator<Task> getAssignedTasksToOperator(String operatorId)
            throws OperatorNotFoundException, NoAssignedTasksException {
        // 1. Buscamos al operador
        Operator op = this.operators.get(operatorId);
        if (op == null) {
            throw new OperatorNotFoundException("Operator " + operatorId + " not found.");
    ...
    @Override
    public Iterator<Task> getAllDoneTasksByOperator(String operatorId)
            throws OperatorNotFoundException, NoCompletedTasksException {
        // 1. Buscamos al operador
        Operator op = this.operators.get(operatorId);
        if (op == null) {
            throw new OperatorNotFoundException("Operator " + operatorId + " not found.");
    ...
    @Override
    public Iterator<Task> getRejectedTasksByOperator(String operatorId)
            throws OperatorNotFoundException, NoRejectedTasksException {
        // 1. Buscamos al operador
        Operator op = this.operators.get(operatorId);
        if (op == null) {
            throw new OperatorNotFoundException("Operator " + operatorId + " not found.");
    ...
    public Operator getOperatorRejectsTheMost() throws OperatorNotFoundException {
        ...
        while (it.hasNext()) {
            ...
            if (currentRejections > maxRejections) {
                ...
                operatorMax = currentOperator;
            } else if (currentRejections == maxRejections) {
                // En caso de empate, nos quedamos con el de ID menor alfabéticamente
                if (operatorMax != null && currentOperator.getId().compareTo(operatorMax.getId()) < 0) {
                    operatorMax = currentOperator;
                }
            }
        }
